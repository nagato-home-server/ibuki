...
現段階では、VPPプロセス分離、YAML socket入力、生成計画のnode dispatch、LAN/underlay attachment生成、双方向GRE構成、strongSwanのIKE/CHILD_SA確立、および暗号化データパスの双方向LAN pingを実ランナーで確認した。[2026-09-24のGitHub Actions実行](https://github.com/nagato-home-server/ibuki/actions/runs/36015737531)では両方向とも3/3応答、損失0%で、site-aのXFRM ESP送信・受信シーケンスは各6まで進んだ。停止後残留と再適用の明示的な検査、VPP Native IPsec backendの実通信は別途確認する。ローカルArchにVPPをビルド・導入せず、手動実行の[GitHub Actions VPP namespace smoke](../.github/workflows/vpp-namespace-smoke.yml)でUbuntu 24.04にFD.io VPPパッケージを導入して検証する。
