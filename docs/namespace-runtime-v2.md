...
`198.18.1.1` と `198.18.2.1` はVPPが生成するGRE outer endpointであり、Linux側のunderlay peerを経由して既存の `a-direct`/`b-direct` へ転送する。strongSwanはIKEを `203.0.113.10`/`203.0.113.9` で確立し、GRE outerの `/32[gre]` をtransport selectorとして保護する。
