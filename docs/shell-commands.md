...
| `scripts/vm-gre-over-ipsec-smoke.sh` | `sudo sh scripts/vm-gre-over-ipsec-smoke.sh samples/gre-over-ipsec.yaml` | `[yaml]` | `BUILD_DIR`, `OUT_DIR` | GRE over IPsecのnamespace charon、SA、VPP GRE、LAN間pingを一括確認する。 |
| `scripts/vm-gre-vpp-data-smoke.sh` | `sudo sh scripts/vm-gre-vpp-data-smoke.sh samples/gre-vpp-data-plane.yaml` | `[yaml]` | `BUILD_DIR`, `OUT_DIR`, `VPPCTL` | コントローラ生成のVPP計画を適用し、双方向VPP GREとLAN実データパスを確認する。strongSwan/XFRMは含めない。 |
...
| `scripts/vm-vpp-ns-topology.sh` | `sudo sh scripts/vm-vpp-ns-topology.sh setup|status|clean` | `setup`, `status`, `clean` | `VPP_NS_RUN_BASE`, `VPPCTL` | 各namespace内にLAN/underlay veth、VPP host-interface、双方向GREを構成する。 |
